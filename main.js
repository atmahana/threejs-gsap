...
function init() {
  ...
  scene.fog = new THREE.Fog(COLORS.light, 10, 15);
...
//   directionalLight.target.position.set();
  //   const dlHelper = new THREE.DirectionalLightHelper(directionalLight, 2, "black");
  //   dlHelper.update();
  //   scene.add(dlHelper);
  ...
  const toLoad = [
    { name: "eva01", group: new THREE.Group(), file: "/eva-01.glb" },
    { name: "eva02", group: new THREE.Group(), file: "/eva-02.glb" },
  ];

  function setupAnimation() {
    models.eva02.position.x = 100;
    models.eva01.position.z = -3;
  ...
  function desktopAnimation() {
    ...
    // Section 2
    tl.to(camera.position, { x: 2, y: 5, z: 2 }, ".second");
    ...
    // // Section 4
    // section += 1;
    // tl.to(camera.position, { x: 5, y: 5, z: 0 }, section);

    // // Section 5
    // section += 1;
    // tl.to(camera.position, { x: 0, y: 5, z: 55}, section);

    // // Section 6
    // section += 1;
    // tl.to(camera.position, { x: 1, y: 5, z: 0 }, section);
  }
